...
export const Route = createFileRoute('/')({
  loader: async () => {
    const shows = await convex.query('shows:getAllShows')
    const progress = await convex.query('progress:getProgress')
...
function TrackerPage() {
  ...
  const [activeTab, setActiveTab] = useState<'rider' | 'sentai' | 'metal'>('rider')
  ...
  const showsByEra = useMemo(() => {
    const currentFranchise = shows.filter((s: any) => s.franchise === activeTab)
        ...
        {(['rider', 'sentai', 'metal'] as const).map((f) => (
          <button
            ...
            className={`pb-2 px-4 capitalize ${
              activeTab === f ? 'border-b-2 border-green-500 text-green-400' : 'text-gray-500'
            }`}
          >
            {f}
          </button>
              ...
              {(eraShows as any[]).map((show) => {
                ...
                return (
                  <button
                    key={show.id}
                    onClick={async () => {
                      await convex.mutation('progress:toggleShow', {
                        showId: show.id,
                        watched: !isWatched,
                      })
                      navigate.invalidate()
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-5 h-5 rounded border flex items-center justify-center ${
                          isWatched ? 'bg-green-500 border-green-500' : 'border-gray-600'
                        }`}
                      >
